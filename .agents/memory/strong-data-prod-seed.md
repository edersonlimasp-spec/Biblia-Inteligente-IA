...
- **Regra:** uma lacuna em `strong_entries` não deve depender da IA para ser reparada; consultar primeiro o léxico completo embutido e persistir a entrada encontrada.
- **Why:** produção pode ter contagem aparentemente suficiente e ainda faltar uma entrada individual; se a geração por IA falhar, uma palavra válida vira 404 mesmo existindo no léxico empacotado.
- **How to apply:** qualquer fluxo de consulta Strong deve preferir banco, depois fonte embutida autoritativa e só então enriquecimento por IA.
