...
function ModuleCard({ 
  ...
  return (
    <motion.div
      ...
      data-testid={testId}
    >
      <div className={`relative overflow-hidden rounded-xl ${gradient} p-2.5 sm:p-3 h-full min-h-[80px] flex flex-col shadow-lg border border-white/10 dark:border-white/5`}>
        <div className="absolute top-0 right-0 w-12 h-12 opacity-10 pointer-events-none">
          <Icon className="w-full h-full" />
        </div>
        
        <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-lg ${iconColor} flex items-center justify-center mb-1.5 shadow-md flex-shrink-0`}>
          <Icon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white" />
        </div>
        
        <div className="flex-1 flex flex-col justify-end relative z-10 min-w-0">
          <div className="flex items-center gap-1 mb-0.5 flex-wrap">
            <h3 className="text-[13px] sm:text-base font-bold text-white leading-tight break-words">{title}</h3>
            {badge && (
              <Badge variant="secondary" className="text-[9px] sm:text-[10px] px-1 py-0 bg-white/20 text-white border-0 flex-shrink-0">
                {badge}
              </Badge>
            )}
          </div>
          <p className="text-[10px] sm:text-xs text-white/75 line-clamp-2 break-words leading-snug">{description}</p>
        </div>
      </div>
    </motion.div>
  );
      ...
      >
        <div className="max-w-2xl mx-auto px-3 py-2 flex items-center justify-between gap-2">
          <div className="flex-1 min-w-0">
            <h1 className="text-base font-serif font-bold bg-gradient-to-r from-primary via-blue-500 to-primary bg-clip-text text-transparent truncate">
              Bíblia Inteligente
            </h1>
