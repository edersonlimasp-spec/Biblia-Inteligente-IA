...
- **Mobile App Distribution:** Configured with Capacitor for iOS and Android app store deployment, including native splash screen, status bar, app lifecycle, keyboard, and haptics plugins.
  - **PWA favicon was a phone screenshot (May 2026, root cause of the "X" the reviewer saw on the car browser):** `client/public/favicon.png` was an 83x180 portrait phone screenshot (status bar visible at "21:03", icon centered) instead of a square icon. When rendered at 16×16/32×32 in browser tabs (and on the Tesla/car infotainment browser the user shared), the tiny scaled-down screenshot looked like a blurry blue X-shape — exactly the "ícone na tela de início" Google flagged. Fix: regenerated `favicon.png` (64×64), `favicon.ico` (multi 16/32), `pwa-icons/icon-192.png` (192×192) and `pwa-icons/icon-512.png` (512×512) — all from `android/app/src/main/res/mipmap-xxxhdpi/ic_launcher.png` via `magick ... -resize NxN -filter Lanczos PNG24:...`. `client/index.html` now declares `favicon.ico`, `favicon.png` (64), `icon-192.png` (192), `icon-512.png` (512) all with `?v=2` cache-bust, plus `apple-touch-icon` with the same cache-bust. `manifest.json` icons also use `?v=2` and `purpose: any` (was `maskable any`, which let Android crop them aggressively). Also generated `public/biblia-store-icon-512.png` for the Play Console Store Listing high-res icon upload.
  - **Google Play icon mismatch (May 2026, third rejection — store-listing/launcher inconsistency):** Reviewer flagged "Ícone do aplicativo" (correct Bíblia book) vs "Ícone na tela de início" (showing the default Replit "X" placeholder). Root cause: AABs **v1.0.11 through v1.0.16** were shipped with the unmodified Capacitor/Replit default launcher icon (light blue X on rounded white background) — confirmed by extracting `mipmap-xxxhdpi/ic_launcher.png` from each historical AAB and comparing md5 (`9e029293ab1ae8e3a6a7b7d0b7177e46` for v11–v16, `4c485b40e5fdc055f35d771a639a0825` for v17–v19, both new and book-correct from v17 onward). Versions v1.0.17+ (and the AABs in `public/`) embed the correct book launcher icon at all 5 densities (mdpi 48 → xxxhdpi 192) plus adaptive `ic_launcher_foreground.png` (108 → 432) over `ic_launcher_background` color `#1A5299`. Two-part fix: (1) the Store Listing high-res 512x512 icon in Play Console may still be the X — replace it with `public/biblia-store-icon-512.png` (generated by `magick mipmap-xxxhdpi/ic_launcher.png -resize 512x512 PNG24:...`); (2) the device launcher icon only updates when the user upgrades from v1.0.16 to ≥ v1.0.17, so promote v1.0.21 to production track to make the X disappear from real devices and from Google Play's automated review screenshots.
  ...
  - **CORS preflight fix (Apr 30 2026, root cause of second Google Play rejection — shipped as versionCode 21 / v1.0.21 because Play Console required a higher code than the previous v1.0.20 upload):** Even after v1.0.19 routed requests to the production URL, the chapter fetch in `BibleReader.tsx` was sending `Cache-Control: no-cache, no-store, must-revalidate` and `Pragma: no-cache` request headers. From the WebView these are non-CORS-safelisted, so the browser issued an OPTIONS preflight whose `Access-Control-Request-Headers` was `cache-control,pragma,authorization,x-device-id`. The server CORS middleware in `server/index.ts` only advertised `Content-Type, Authorization, x-device-id, x-client-platform, x-bootstrap-token` in `Access-Control-Allow-Headers`, so the browser blocked the actual GET, fetch rejected, and the user saw "Erro ao carregar capítulo" again. Two-sided fix: (1) server now echoes the requested headers back in the preflight response (with the legacy superset as fallback) and adds `Access-Control-Request-Headers` to `Vary`, so any future header added by a query will not break the API; (2) `BibleReader.tsx` chapter queryFn no longer sends `Cache-Control` / `Pragma` headers — the existing `cache: 'no-store'` fetch option is enough to bypass the WebView cache without triggering the extra preflight. Test: `curl -X OPTIONS -H 'Origin: https://localhost' -H 'Access-Control-Request-Headers: cache-control,pragma,authorization,x-device-id' https://bibliainteligente.replit.app/api/bible/psa/2` must return `Access-Control-Allow-Headers: cache-control,pragma,authorization,x-device-id`. Note: deploying the server fix alone is enough to make the existing v1.0.19 installs (and the Google Play review queue) start working; v1.0.20 is shipped only as defense-in-depth. To rebuild the AAB, run on a host with SDK installed: `npm run build && npx cap sync android && cd android && ./gradlew bundleRelease` (the Replit dev container kills detached Gradle daemons before the bundle finishes).
