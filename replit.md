...
- **Mobile App Distribution:** Configured with Capacitor for iOS and Android app store deployment, including native plugins and optimized icons for a native-like experience. This involves careful management of app icons, splash screens, and API routing for native environments to ensure seamless functionality and compliance with store review guidelines.
  - **Android:** publicado no Google Play (`applicationId: app.replit.bibliainteligente.twa`). Versão atual no projeto: `versionCode 27 / versionName 1.0.27` (corrige (a) header "Bíblia Inteligente IA" que reaparecia após o splash — agora `postSplashScreenTheme` aponta para `AppTheme.NoActionBar`; (b) compras nativas no Google Play via `cordova-plugin-purchase` v13 + permissão `com.android.vending.BILLING`). NÃO alterar `applicationId`.
  - **Google Play Billing:** integração via `cordova-plugin-purchase` (CdvPurchase v13) em `client/src/lib/inAppPurchases.ts`. Produtos registrados (devem existir idênticos no Google Play Console): `gold_monthly`, `gold_annual`, `premium_monthly`, `premium_annual` (PAID_SUBSCRIPTION) e `strong_lifetime` (NON_CONSUMABLE). Verificação server-side em `POST /api/iap/verify/google` exige o secret `GOOGLE_PLAY_SERVICE_ACCOUNT_KEY` em produção (sem ele, o backend recusa a compra).
  - **iOS:** preparado para TestFlight/App Store (`bundleId: com.bibliainteligente.ios`). Pendente de execução manual no Xcode: 1) habilitar capability "Sign in with Apple" no Apple Developer Portal e em Signing & Capabilities; 2) `pod install` em `ios/App`; 3) configurar Team/Provisioning Profile.
