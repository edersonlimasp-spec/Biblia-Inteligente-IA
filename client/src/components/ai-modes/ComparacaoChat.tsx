...
export function ComparacaoChat({ onBack, onNavigateToSubscriptions }: ComparacaoChatProps) {
  ...
  useEffect(() => {
    ...
    const savedHistory = localStorage.getItem('comparacao_chat_history');
    if (savedHistory) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('comparacao_chat_history', JSON.stringify(savedConversations));
  }, [savedConversations]);
  ...
  return (
    <div className="h-screen bg-gradient-to-b from-amber-950/20 to-background flex flex-col overflow-hidden">
      {/* Compact Premium Header with gradient (fixed) */}
      <header className="flex-shrink-0 z-50 bg-gradient-to-r from-amber-600 via-orange-500 to-amber-600 text-white shadow-lg">
        <div className="max-w-6xl mx-auto px-3 py-2 flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-amber-100 truncate">Perspectivas denominacionais</p>
          </div>
          <div className="flex items-center gap-1">
            ...
            <Badge className="bg-gradient-to-r from-amber-300 to-orange-400 text-amber-900 border-0 text-xs px-2 py-0.5 font-semibold">
              <Crown className="w-3 h-3 mr-1" /> Premium
            </Badge>
          </div>
        </div>
      ...
      {/* Compact Premium Input area (fixed at bottom) */}
      <div className="flex-shrink-0 border-t bg-gradient-to-r from-amber-50/50 to-orange-50/50 dark:from-amber-950/20 dark:to-orange-950/20 p-2">
        <div className="max-w-4xl mx-auto">
          <AnimatePresence>
            {pendingImage && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mb-2">
                <div className="relative inline-block">
                  <img src={pendingImage.url} alt="Preview" className="h-12 rounded-lg border-2 border-amber-300" />
          ...
          <div className="flex gap-1.5 items-end">
            ...
            <Button variant="outline" onClick={() => fileInputRef.current?.click()} disabled={isPending} className="h-8 w-8 p-0 flex-shrink-0 border-amber-300" data-testid="button-attach">
              <ImageIcon className="w-4 h-4 text-amber-600" />
            </Button>
            <Textarea
              placeholder="Compare posições teológicas..."
              value={input}
              onChange={(e) => setInput(e.target.value)}
              onKeyDown={handleKeyPress}
              rows={1}
              className="resize-none min-h-[32px] max-h-24 flex-1 text-sm py-1.5 px-2 border-amber-200 focus-visible:ring-amber-500"
              data-testid="input-message"
            />
            <Button onClick={handleSubmit} disabled={(!input.trim() && !pendingImage) || isPending} className="h-8 w-8 p-0 flex-shrink-0 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700" data-testid="button-send">
